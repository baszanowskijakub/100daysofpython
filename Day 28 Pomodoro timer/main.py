...
def count_down(count):
    ...
    if count > 0:
        ...
        for _ in range(work_sessions):
            marks += "✓"
        check_marks.config(text=marks, font=20)
...
start_button = Button(text="Start", highlightthickness=0, command=start_timer,
       fg="white", bg=GREEN, font=("Helvetica", 12, "bold"), padx=10, pady=5, borderwidth=0)
start_button.grid(column=1, row=3)

reset_button = Button(text="Reset", highlightthickness=0, command=reset_timer, fg="white", bg=GREEN, font=("Helvetica", 12, "bold"),
